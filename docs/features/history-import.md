...
- **An occurrence the export has already paid is never projected again.**
  Each posted row is matched to the occurrence it is nearest to, so a
  payment that arrived early or late settles the occurrence it belongs to
  and not its neighbour. A row more than half an interval from every
  occurrence matches none: it is a one-off at that payee, not a payment of
  the stream.
