...
The prompt also describes the checkbox by name, so **a change to the
checkbox's wording is a change to what the explainer tells people to do.**
When the label collapsed from two readings to one — "Can't do this" on every
row, empty until ticked — `prompt.py` went on describing a control that no
longer existed, in two places, and the offers rewrite reintroduced it in a
third. `test_chat.py` pins the brief and the ruled-out context against the
real label now. The checkbox is TypeScript and the prompt is Python; nothing
else connects them.
