...
- [ ] Refresh the two numbers in the build section. The suite keeps growing and
      ...
      As of Sat 14:00 it is 1,325 backend tests plus 216 frontend tests, and the
      three demo accounts solve in 3.9, 5.7 and 7.7 ms (100 random accounts
      average 2.6 ms each). Measured with `.venv/bin/pytest backend/ -m perf -s`.
